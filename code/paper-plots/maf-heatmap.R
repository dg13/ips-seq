...
box1 <- c(0.05,0.4,0,0.001)
box2 <- c(0,0.24,0.3,0.7)
## wgsFile <- "Data/newCalls2/rdas/mut-wgs-fdr5.collapseDinucs.unfiltered.rda"
## wesFile <- "Data/newCalls2/rdas/mut-wes-fdr5.collapseDinucs.unfiltered.rda"
## outFile1 <- "plots/paperFigures/mafHeatMap-wgs.png"
## outFile2 <- "plots/paperFigures/mafHeatMap-wes.png"
lwd <- 1.7
afThresh <- 0.7
...
br <- c(0,seq(0,1,0.02))
cex <- 1.7
message("Reading ",wgsFile)
m <- readRDS(wgsFile)
message("Plotting to ",outFile1)
png(file=outFile1,res=250,units="in",hei=5,wid=5)
...
br <- c(0,seq(0,1,0.02))
cex <- 1.7
message("Reading ",wesFile)
m <- readRDS(wesFile)
message("Plotting to ",outFile2)
png(file=outFile2,res=250,units="in",hei=5,wid=5)
layout(matrix(1:4,nrow=2),wid=c(1,3),hei=c(3,1))
par(mar=c(2,3.8,1,0),mgp=c(2.4,0.5,0),tcl=-0.25,cex.axis=cex,cex.lab=cex)
r <- hist(m$iaaf,br=br,plot=F)
cols <- rep(grey(0.6),length(r$breaks))
